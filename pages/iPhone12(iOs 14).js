  ...
  touchMov: function(config){
    ...
    el.on('pointerdown', function (e) {
      ...
      el.on('pointerup pointercancel', function (ex) {
        ...
        el.off('pointermove pointerup pointercancel');
      });
    });
...
<div class="tableContent"></div>
    ...
    globalState.dateTime.days.map(day =>
      header.append(`<div class="dayName">${config.fullDay ? day : day.charAt(0)}</div>`)
    );
    for (var k = 0; k <= firstDay; k++) content.prepend('<div></div>');
    for (let i = 1; i <= daysMonth; i++) {
      content.append(`<div class="dayNum ${today == i ? 'active':''}">${i}</div>`);
    }
    return this;
  },
  ...
  dateIcon: function(config){
    ...
    this.append(
      `<div class="dateWrapper">
        <p class="dayNam">${config.fullDay ? day : day.substring(0,3)}</p>
        <p class="dayNum">${today}</p>
      </div>`
    );
    return this;
  },
  ...
  clock: function(){
    ...
    let numbers = '';
    for (let i = 1; i <= 12; i++) {
      numbers += `<div class="number" data-num="${i}"></div>`;
    }
...
<div class="hand seconds" style="transform: rotate(${transformSeconds});"><div class="stick"></div></div>
  ...
  hour: function(config){
    config = jQuery.extend({ realtime: true }, config);
    let update = () => {
      let d = new Date();
    ...
    update();
    if (config.realtime) setInterval(update, 1000);
    return this;
  },
  ...
  date: function(config){
    ...
    let d = config.date;
    let day = globalState.dateTime.days[d.getDay()];
    let month = globalState.dateTime.months[d.getMonth()];
    this.text(`${config.fullDay ? day : day.substring(0,3)}, ${d.getDate()} ${month}`);
    return this;
  }
});
    ...
    if (!$('.youTubeApp').length) {
      $('.mainScreen').append(`
        <div class="youTubeApp hidden">
	     <div class="appScreen">
            <div class="contents">
	            <iframe width="290" height="550" src="https://khanhpc-five.vercel.app/yt.html">
</iframe>
			</div>
		 </div>
        </div>`
